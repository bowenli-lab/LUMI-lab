...
class LiquidSamplerDAO(BaseDAO):
    device_id: ObjectId
    ...
    def __init__(self, device_id: ObjectId):
        ...
        self.liquid_sampler_object_collection = self.client["devices"]
        self.liquid_sampler_execution_collection = self.client["devices_execution"]

    def create_entry(self, object_name: str, status, **kwargs) -> ObjectId:
        self.object_name = object_name

        # if exist the same object_name then update the status
        query_res = self.liquid_sampler_object_collection.find_one({"object_name": object_name})
        if query_res:
        ...
        try:
            self.liquid_sampler_object_collection.insert_one(entry)
    ...
    def log_step(self,
        ...
        self.liquid_sampler_execution_collection.insert_one(entry)

    def update_status(self, status):
        (self.liquid_sampler_object_collection.

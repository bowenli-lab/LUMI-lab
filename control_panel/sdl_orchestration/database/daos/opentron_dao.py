...
class OpentronDAO(BaseDAO):
    ...
    def __init__(self, device_id: ObjectId,
        ...
        self.opentron_object_collection = self.client["devices"]
        self.opentron_execution_collection = self.client["devices_execution"]
    ...
    def create_entry(self, object_name: str, status, **kwargs) -> ObjectId:
        ...
        query_res = self.opentron_object_collection.find_one({"object_name": object_name})
        if query_res:
        ...
        try:
            self.opentron_object_collection.insert_one(entry)
    ...
    def log_step(self, step: str, task_id: Optional[ObjectId],
            ...
            self.opentron_execution_collection.insert_one(entry)

    def update_status(self, status):
        self.opentron_object_collection.update_one({"_id": self.device_id},

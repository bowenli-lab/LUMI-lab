...
class FeederDAO(BaseDAO):
    ...
    def __init__(
        ...
        self.feeder_object_collection = self.client["devices"]
        self.feeder_execution_collection = self.client["devices_execution"]
    ...
    def create_entry(self, object_name: str, status, **kwargs) -> ObjectId:
        ...
        query_res = self.feeder_object_collection.find_one({"object_name": object_name})
        if query_res:
        ...
        try:
            self.feeder_object_collection.insert_one(entry)
    ...
    def log_step(
        ...
        self.feeder_execution_collection.insert_one(entry)

    def update_status(self, status):
        (
            self.feeder_object_collection.update_one(
                {"_id": self.device_id},
                {
                    "$set": {
                        "status": str(status),

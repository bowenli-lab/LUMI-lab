...
class ClampDAO(BaseDAO):
    ...
    def __init__(
        ...
        self.clamp_object_collection = self.client["devices"]
        self.clamp_execution_collection = self.client["devices_execution"]

    def create_entry(self, object_name: str, status, **kwargs) -> ObjectId:
        self.object_name = object_name

        # if exist the same object_name then update the status
        query_res = self.clamp_object_collection.find_one({"object_name": object_name})
        if query_res:
        ...
        try:
            self.clamp_object_collection.insert_one(entry)
    ...
    def log_step(
        ...
        self.clamp_execution_collection.insert_one(entry)

    def update_status(self, status):
        (
            self.clamp_object_collection.update_one(
                {"_id": self.device_id},
                {
                    "$set": {
                        "status": str(status),

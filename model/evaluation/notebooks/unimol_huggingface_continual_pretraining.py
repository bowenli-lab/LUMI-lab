...
# ===== config =====
os.environ["WANDB_PROJECT"] = "molecular-property-pretrain"  # name your W&B project
weight_path = "/scratch/ssd004/datasets/cellxgene/3d_molecule_save/weights/mol_pre_no_h_220816.pt"
dict_path = "/h/pangkuan/dev/SDL-LNP/model/evaluation/notebooks/dict.txt"
seed = 0
...
run_name = f"lipo_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
epoch_num = 6
warmup_ratio = 0.06
...
trainer.save_model(f"/scratch/ssd004/scratch/pangkuan/uni-mol-weights/{run_name}")

# save model backbone
torch.save(
    trainer.model.model.state_dict(),
    f"/scratch/ssd004/scratch/pangkuan/uni-mol-weights/{run_name}/backbone.pt",
)

...
@register_loss("unimol_weak_supervision")
class UniMolWeakSupervisionLoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if weak_sup_logits is not None:
            # apply mse loss to weak supervision signal
            targets = sample[target_key]
            for label in self.args.target_label:
                # check if the target is nan
                gt = targets[label]
                idx_nan = torch.isnan(gt)
                
                # replace nan with pred
                targets[label][idx_nan] = weak_sup_logits[label][idx_nan].squeeze(-1).float()
                
                weak_sup_loss = F.mse_loss(
                    weak_sup_logits[label].squeeze(-1).float(),
                    targets[label],
                    reduction="mean",
                )
                
                loss = loss + weak_sup_loss * self.args.weak_sup_loss
                logging_output[f"weak_sup_loss_{label}"] = weak_sup_loss.data            
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        for label in list(logging_outputs[0].keys()):
            if "weak_sup_loss" in label:
                weak_sup_loss = sum(
                    log.get(label, 0) for log in logging_outputs
                )
                metrics.log_scalar(
                    label,
                    weak_sup_loss / sample_size,
                    sample_size,
                    round=3,
                )
...
@register_loss("unimol_infer_weak_supervision")
class UniMolInferLossWeakSupervision(UnicoreLoss):
    def __init__(self, task):
        super().__init__(task)
        self.padding_idx = task.dictionary.pad()

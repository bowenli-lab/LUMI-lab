...
class RnCLoss(nn.Module):
    ...
    def forward(self, features, labels):
        ...
        # affine transform feature
        noise = (
            torch.randn_like(features) * 0.5 + 0
        )  # std_dev and mean can be set as needed

        # normalize
        perturbed_feature = F.normalize(features + noise, p=2, dim=-1)

        features = torch.cat([features, perturbed_feature], dim=0)  # [2bs, feat_dim]

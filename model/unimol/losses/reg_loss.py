...
class FinetuneMSELoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if not self.training:
            if hasattr(self.task, "mean") and hasattr(self.task, "std"):
...
@register_loss("finetune_mse_noise")
class FinetuneMSE_NOISE_Loss(UnicoreLoss):
    def __init__(self, task):
        super().__init__(task)
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        noise = (
            torch.randn_like(targets) * 0.5 + 0
        )  # std_dev and mean can be set as needed
...
@register_loss("finetune_rank")
class FinetuneRankMSELoss(UnicoreLoss):
    def __init__(self, task):
        super().__init__(task)
        self.rncloss = RnCLoss()
    ...
    def forward(self, model, sample, reduce=True):
        ...
        reg_output = net_output[0]
        feature_output = net_output[1]
        loss = self.compute_loss(model, reg_output, sample, reduce=reduce)
        loss = loss + 2 * self.rncloss(
            feature_output, sample["target"]["finetune_target"]
        )
        sample_size = sample["target"]["finetune_target"].size(0)
        if not self.training:
            if hasattr(self.task, "mean") and hasattr(self.task, "std"):

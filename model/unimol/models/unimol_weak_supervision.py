...
@register_model("unimol_weak_supervision")
class UniMolModelWeakSupervision(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args, dictionary):
        super().__init__()
        base_architecture_weaksup(args)
        self.args = args
        ...
        self.target_label_predictor = nn.ModuleDict({
            label: ClassificationHead(
                input_dim=self.args.encoder_embed_dim,
                inner_dim=self.args.encoder_embed_dim,
                num_classes=1,
                activation_fn=self.args.pooler_activation_fn,
                pooler_dropout=self.args.pooler_dropout,) for label in self.target_label
        })
        ...
        **kwargs,
        ...
        if self.target_label is not None:
            for label in self.target_label:
                weak_sup_logits[label] = self.target_label_predictor[label](encoder_rep)
        ...
        if not features_only:
            ...
            if self.args.masked_coord_loss > 0:
                ...
                # Mask padding
                pair_coords_mask = (1 - padding_mask.float()).unsqueeze(-1) * (
        ...
        if ("mode" not in vars(self.args)) or (self.args.mode == "infer"):
...
@register_model_architecture("unimol_weak_supervision", "unimol_weak_supervision")
def base_architecture_weaksup(args):
...
@register_model_architecture("unimol_weak_supervision", "unimol_weak_supervision_base")
def unimol_base_architecture_weaksup(args):
    base_architecture_weaksup(args)

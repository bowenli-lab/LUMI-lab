...
class MergedLMDBDatasetWithSampling:
    ...
    def __len__(self):
        ...
        for idx, keys in enumerate(self._keys_list):
            sum_length += int(len(keys) * self.data_weights[idx])
        return sum_length
    ...
    def __getitem__(self, idx):
        if not hasattr(self, "env_list"):
            self.env_list = [self.connect_db(db_path) for db_path in self.db_path_list]
        # sample the dataset based on the weights
        db_index = np.random.choice(len(self.db_path_list), p=self.data_weights)
        # get a random key from the selected dataset
        key = random.choice(self._keys_list[db_index])
...
@register_task("unimol_mixed_sampling")
class UniMolMixedSamplingTask(UnicoreTask):
    ...
    def __init__(self, args, dictionary):
        ...
        # self.data_weights = [x / sum(self.data_weights) for x in self.data_weights]
        assert sum(self.data_weights) == 1, "Data weights must sum to 1"

...
#SBATCH --job-name=mixed-sampling
#SBATCH --mem=120GB
#SBATCH --gres=gpu:4
...
run_name=mixed-sampling
save_dir=/fs01/datasets/cellxgene/3d_molecule_save/pretrain-${run_name}/
...
/h/pangkuan/miniconda3/envs/unimol/bin/python -m torch.distributed.launch --nproc_per_node=$n_gpu --master_port=$MASTER_PORT /h/pangkuan/miniconda3/envs/unimol/bin/unicore-train $data_path --user-dir ./unimol --train-subset train --valid-subset valid \
    ...
    --task unimol_mixed_sampling --loss unimol --arch unimol_base \
    ...
    --save-dir $save_dir --only-polar $only_polar --mode train --data-weights 0.9 0.1

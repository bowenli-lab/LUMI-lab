...
#SBATCH --job-name=weak-sup-pretrain
#SBATCH --mem=120GB
#SBATCH --gres=gpu:4
...
data_path=/scratch/ssd004/datasets/cellxgene/3d_molecule_data/cleaned_ligands,/scratch/ssd004/datasets/cellxgene/3d_molecule_data/15m-lib,/scratch/ssd004/datasets/cellxgene/3d_molecule_data/chembl/conformation
target_label=most_apka,most_bpka,logp,logd
smi_idx_mapping=/scratch/ssd004/datasets/cellxgene/3d_molecule_data/weak-supervision
property_prediction_csv=/scratch/ssd004/datasets/cellxgene/3d_molecule_data/chembl
run_name=weak-supervision
save_dir=/fs01/datasets/cellxgene/3d_molecule_save/${run_name}/
...
/h/pangkuan/miniconda3/envs/unimol/bin/python -m torch.distributed.launch --nproc_per_node=$n_gpu --master_port=$MASTER_PORT /h/pangkuan/miniconda3/envs/unimol/bin/unicore-train $data_path --user-dir ./unimol --train-subset train --valid-subset valid \
    ...
    --task unimol_weak_supervision --loss unimol_weak_supervision --arch unimol_weak_supervision_base \
    --optimizer adam --adam-betas "(0.9, 0.99)" --adam-eps 1e-6 --clip-norm 1.0 --weight-decay $wd \
    --lr-scheduler polynomial_decay --lr $lr --warmup-updates $warmup_steps --total-num-update $max_steps \
    --update-freq $update_freq --seed $seed \
    --fp16 --fp16-init-scale 4 --fp16-scale-window 256 --tensorboard-logdir $save_dir/tsb \
    --max-update $max_steps --log-interval 1000 --log-format simple \
    --save-interval-updates 10000 --validate-interval-updates 10000 --keep-interval-updates 5 --no-epoch-checkpoints \
    ...
    --save-dir $save_dir --only-polar $only_polar --mode train --target-label $target_label \
    --smi-idx-mapping $smi_idx_mapping --property-prediction-csv $property_prediction_csv \
    --weak-sup-loss $weak_sup_loss

...
data_path=/home/sdl/vector_cellxgene_data/cleaned_ligands,/home/sdl/vector_cellxgene_data/15m-lib
date_time=$(date "+%Y%m%d-%H%M")
save_dir=/home/sdl/3d_molecule_save/contrastive-conitnual-10x-mixed-annealing/
n_gpu=1
MASTER_PORT=10086
lr=5e-5
wd=1e-4
...
~/miniconda3/envs/unimol/bin/python -m torch.distributed.launch --nproc_per_node=$n_gpu --master_port=$MASTER_PORT ~/miniconda3/envs/unimol/bin/unicore-train $data_path --user-dir ./unimol --train-subset train --valid-subset valid \
    ...
    --save-dir $save_dir --only-polar $only_polar --mode train --contrastive-loss $contrastive_loss --data-weights 0.2 1 2>&1 | tee $save_dir/train.log

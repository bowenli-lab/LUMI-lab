...
# metadata
metadata = {
    'protocolName': "4CR LNP to Cell - Round Bottom Left + Flat Bottom Right - higher dispense",
    'author': 'Kevin <kevinchang.wang@mail.utoronto.ca',
    'description': 'Opentrons liquid dispensing protocol to add LNPs to 2 cell lines, the first (left) in a round-bottom plate, and the second (right) in a flat-bottom plate',
    'apiLevel': '2.12'
}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # load lnp plates
    lnp_plate_1 = protocol.load_labware('servicebio_96_wellplate_200ul', location=lnp_plate_1_location,
                                        label='LNP_Plate_1')
    lnp_plate_2 = protocol.load_labware('servicebio_96_wellplate_200ul', location=lnp_plate_2_location,
                                        label='LNP_Plate_2')

    # load cell plates
    cell_plate_1a = protocol.load_labware('biomartubottom_96_wellplate_200ul', location=cell_plate_1a_location,
                                          label='Cell_Plate_1a')
    cell_plate_1b = protocol.load_labware('thermoscientific_96_wellplate_200ul', location=cell_plate_1b_location,
                                          label='Cell_Plate_1b')
    cell_plate_2a = protocol.load_labware('biomartubottom_96_wellplate_200ul', location=cell_plate_2a_location,
                                          label='Cell_Plate_2a')
    cell_plate_2b = protocol.load_labware('thermoscientific_96_wellplate_200ul', location=cell_plate_2b_location,
                                          label='Cell_Plate_2b')

    # transfer lnp to cell plates
    p20_multi_pipette.well_bottom_clearance.dispense = 2.5

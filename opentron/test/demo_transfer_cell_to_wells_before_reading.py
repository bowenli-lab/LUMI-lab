...
def run(protocol: protocol_api.ProtocolContext):
    ...
    cold_deepwell = protocol.load_labware("customizedcoolerplate_96_wellplate_1300ul", location="6")
    ...
    right_multi_pipette.transfer(
        50,  # NOTE: 10 ul for test
        cold_deepwell.columns()[4],
        previous_cell_plate.wells()[:8],
        mix_after=(3, 300),
        new_tip="always",
        trash=False,
    )
    ...
    right_multi_pipette.transfer(
        300,  # NOTE: 10 ul for test
        previous_cell_plate.wells()[:8],
        new_cell_plate.wells()[:8],
        mix_after=(3, 300),
        new_tip="always",
        trash=False,
    )
